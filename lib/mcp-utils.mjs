...
export class MCPLogger extends ConsoleLogger {
     ...
     */
    logServerStart(serverName, config) {
     ...
     */
    logToolCall(serverName, toolName, args) {
     ...
     * Log MCP tool result
     */
    logToolResult(serverName, toolName, result) {
...
export class MCPServerStartupError extends MCPError {
    constructor(serverName, originalError) {
        super(`Failed to start MCP server: ${serverName}`, serverName, originalError);
...
export class MCPToolExecutionError extends MCPError {
    constructor(serverName, toolName, originalError) {
        super(`Failed to execute tool ${toolName} on server ${serverName}: ${originalError.message}`, serverName, originalError);
 ...
 */
export function handleMCPError(error, logger) {
