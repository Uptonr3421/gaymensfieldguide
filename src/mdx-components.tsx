...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    p: ({ children }) => <p className="text-lg leading-relaxed text-zinc-600 dark:text-zinc-300 mb-6 font-medium">{children}</p>,
    ul: ({ children }) => <ul className="list-disc list-outside ml-6 mb-6 space-y-2 text-zinc-600 dark:text-zinc-300">{children}</ul>,
    ol: ({ children }) => <ol className="list-decimal list-outside ml-6 mb-6 space-y-2 text-zinc-600 dark:text-zinc-300">{children}</ol>,
    li: ({ children }) => <li className="pl-2">{children}</li>,
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-banana-500 pl-6 py-2 my-8 italic text-xl text-zinc-500 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-900/50">
        {children}
      </blockquote>
    ),
    a: ({ href, children }) => (
      <Link href={href as string} className="text-banana-500 dark:text-banana-400 font-bold hover:underline decoration-2 underline-offset-2">
        {children}
      </Link>
    ),
    img: (props) => (
